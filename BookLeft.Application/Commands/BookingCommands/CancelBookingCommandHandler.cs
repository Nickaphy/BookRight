...
{   
    ...
    {        
        _ = await _bookingRepo.GetByIdAsync(request.BookingId)
            ?? throw new Exception($"Booking with ID {request.BookingId} not found.");      // Validater om bookingen findes
              
        var booking = await _bookingRepo.GetByIdAsync(request.BookingId); // Henter bookingen, hvis den findes
